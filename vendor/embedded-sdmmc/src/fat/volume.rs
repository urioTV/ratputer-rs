...
impl FatVolume {
        ...
        D: BlockDevice,
    {
        ...
        for (i, dir_entry_bytes) in block.chunks_exact_mut(OnDiskDirEntry::LEN).enumerate() {
            ...
            if dir_entry.is_end() {
                ...
                block[start] = 0xE5;
                // RATPUTER PATCH: also delete the long-file-name entries stored
                // directly before the short entry in this block, otherwise they
                // are left orphaned. (Entries continuing in the previous block
                // are left alone; hosts ignore LFNs whose checksum no longer
                // matches a short entry.)
                let mut lfn = i;
                while lfn > 0 {
                    lfn -= 1;
                    let entry = lfn * OnDiskDirEntry::LEN;
                    if block[entry + 11] != 0x0F || block[entry] == 0xE5 {
                        break;
                    }
                    block[entry] = 0xE5;
                }
        ...
        D: BlockDevice,
    {
        ...
        loop {
            // RATPUTER PATCH: read the successor before clearing this FAT entry,
            // then account every freed cluster including the final one. Upstream
            // skipped the increment when `EndOfFile` broke out of the loop.
            let following = self.next_cluster(block_cache, next);
    ...
    /// RATPUTER PATCH: release every cluster of a deleted file or directory.
    ///
    /// Upstream only marks the directory entry as deleted, which leaks the
    /// whole cluster chain (host `chkdsk` reports it as lost clusters).
        ...
        D: BlockDevice,
    {
        ...
        self.truncate_cluster_chain(block_cache, cluster)?;
        self.update_fat(block_cache, cluster, ClusterId::EMPTY)?;
        match self.next_free_cluster {
            Some(next_free) if next_free.0 <= cluster.0 => {}
            _ => self.next_free_cluster = Some(cluster),
        }
        if let Some(ref mut free_clusters) = self.free_clusters_count {
            *free_clusters += 1;
        }
        Ok(())
    }
